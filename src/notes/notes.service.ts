...
export class NotesService {
  ...
  async create(dto: CreateNoteDto, misc: any): Promise<NoteEntity> {
    this.logger.log(
      `Creating note with title: ${dto.title}, tags: ${JSON.stringify(dto.tags)}`
    );
    ...
    this.logger.log(
      `Note created successfully with id: ${saved.id}, tags: ${JSON.stringify(saved.tags)}, type: ${typeof saved.tags}, isArray: ${Array.isArray(saved.tags)}`
    );
    ...
    if (rawNote) {
      this.logger.log(
        `Raw tags from DB (note_tags): ${rawNote.note_tags}, type: ${typeof rawNote.note_tags}`
      );
    }
  ...
  async findAll(query: ListNotesQueryDto): Promise<ListNotesResponseDto> {
    ...
    this.logger.log(
      `Finding notes - page: ${page}, limit: ${limit}, search: ${search || "none"}, tags: ${tags?.join(",") || "none"}`
    );
